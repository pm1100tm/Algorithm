...
def solution(strings: list[str], n: int) -> list[str]:
    def custom_sort(index: int, x: str):
        if not index:
            return x[index], x[index + 1:]

        return x[index], x[0:index], x[index + 1:]

    answer = sorted(strings, key=lambda x: custom_sort(n , x))
    return answer


def solution2(strings: list[str], n: int) -> list[str]:
...
if __name__ == '__main__':
    ...
    assert solution2(['sun', 'bed', 'car'], 1) == ['car', 'bed', 'sun']
    assert solution2(['abce', 'abcd', 'cdx'], 2) == ['abcd', 'abce', 'cdx']
